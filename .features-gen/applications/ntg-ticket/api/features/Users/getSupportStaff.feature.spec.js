...
test.describe("Support Staff API Validation", () => {
  ...
  test("GET Support Staff returns successful response with required fields", { tag: ["@getSupportStaff", "@ntg-ticket"] }, async ({ When, request, Then, And }) => {
  ...
  test("GET Support Staff response has valid data structure", { tag: ["@getSupportStaff"] }, async ({ When, request, Then, And }) => {
    await When("I send a GET request to endpoint \"getSupportStaff\"", null, { request });
    await Then("the response status should be 200");
  ...
  test("GET Support Staff response fields have non-empty values", { tag: ["@getSupportStaff"] }, async ({ When, request, Then, And }) => {
    await When("I send a GET request to endpoint \"getSupportStaff\"", null, { request });
    await Then("the response status should be 200");
  ...
  test("GET Support Staff response has valid email format", { tag: ["@getSupportStaff"] }, async ({ When, request, Then, And }) => {
    await When("I send a GET request to endpoint \"getSupportStaff\"", null, { request });
    await Then("the response status should be 200");
...
const bddFileMeta = {
  "GET Support Staff returns successful response with required fields": {"pickleLocation":"5:3","tags":["@getSupportStaff","@ntg-ticket"],"ownTags":["@ntg-ticket"]},
  "GET Support Staff response has valid data structure": {"pickleLocation":"15:3","tags":["@getSupportStaff"]},
  "GET Support Staff response fields have non-empty values": {"pickleLocation":"24:3","tags":["@getSupportStaff"]},
  "GET Support Staff response has valid email format": {"pickleLocation":"31:3","tags":["@getSupportStaff"]},
};