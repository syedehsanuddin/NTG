...
test.describe("Adjust stock (Physical count correction)", () => {
  ...
  test("POST Adjust stock (Physical count correction) returns successful response", { tag: ["@postAdjustStock"] }, async ({ When, request, Then }) => {
    await When("I send a POST request to endpoint \"postAdjustStock\" with payload '{\"ingredientId\":\"{STORED_INGREDIENT_ID}\",\"newQuantity\":5000000,\"branchId\":\"{BRANCH_ID}\",\"reason\":\"Automated Test \",\"transactionDate\":\"2026-01-28T10:38:41.244Z\"}' and headers \"language=en\"", null, { request });
    await Then("the response status should be 201");
  });
