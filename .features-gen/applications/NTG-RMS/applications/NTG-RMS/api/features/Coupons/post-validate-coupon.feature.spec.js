...
test.describe("Validate coupon code", () => {
  ...
  test("POST Validate coupon code returns successful response", { tag: ["@postValidateCoupon"] }, async ({ When, request, Then }) => {
    await When("I send a POST request to endpoint \"postValidateCoupon\" with payload '{\"code\":\"{STORED_COUPON_CODE}\",\"subtotal\":5000}' and headers 'branchId=bcf7f7a6-f6a2-474a-8bb9-2ccf10a26191'", null, { request });
    await Then("the response status should be 201");
  });
